 ...
 */
public final class ForemanHostProvider implements HostProvider {

    @Override
    public CompletionStage<Set<String>> getHosts() {
        return getHostPage(1).thenCompose(this::pageComplete);
    }

    private CompletionStage<Set<String>> pageComplete(final HostPageResponse response) {
        if (response.getTotal() > response.getPage() * response.getPerPage()) {
            return getHostPage(response.getPage() + 1, response.getPerPage())
                    .thenCompose(this::pageComplete)
                    .thenApply(set -> Stream.concat(response.getResults().stream().map(ForemanHost::getName), set.stream())
                            .collect(Collectors.toSet()));
        } else {
            return CompletableFuture.completedFuture(response.getResults().stream().map(ForemanHost::getName).collect(Collectors.toSet()));
        }
    }
                ...
                if (jsonNode.isObject()) {
                    ...
                    final ImmutableList<ForemanHost> foremanHosts = ImmutableList.copyOf(
                            foremanHostContainers
                                    .stream()
                                    .map(ForemanHostContainer::getHost)
    ...
    private ForemanHostProvider(final Builder builder) {
    ...
    /**
     * Implementation of the Builder pattern for ForemanHostProvider.
     *
     * @author Brandon Arp (brandon dot arp at smartsheet dot com)
     */
    public static final class Builder extends OvalBuilder<ForemanHostProvider> {
        /**
         * Public constructor.
         */
        public Builder() {
            super(ForemanHostProvider::new);
        }
