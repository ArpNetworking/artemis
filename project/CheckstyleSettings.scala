...
import com.puppycrawl.tools.checkstyle.Main._
import sbt._
import Keys._
  ...
  val checkstyleTask = checkstyle <<=
    (streams, baseDirectory, sourceDirectory in Compile, target) map {
      (streams, base, src, target) =>
        ...
        System.setProperty("header_file", (buildDir / "al2").getAbsolutePath)
        System.setProperty("suppressions_file", (base / "checkstyle-suppressions.xml").getAbsolutePath)
